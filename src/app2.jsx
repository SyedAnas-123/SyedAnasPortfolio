  ...
  "AI & GenAI": [
    {
      ...
      tech: ["Python", "LangChain", "Pinecone", "OpenAI", "AWS EC2", "Docker", "Python", "HuggingFace Embeddings" ,],
      links: {
      ...
      image: "medical-rag.png" // Put this image in your public folder
    }
  ],
  "Computer Vision": [
    {
...
const CustomCursor = () => {
  ...
  useEffect(() => {
    ...
    const moveCursor = (e) => {
      ...
      // Trailer follows with delay (handled by CSS transition mostly, but JS for positioning)
      if (trailer) {
        trailer.animate({
    ...
    // Add bloom effect on hoverables
  ...
  return (
    <>
      <div ref={cursorRef} className="fixed top-0 left-0 w-2 h-2 bg-cyan-400 rounded-full pointer-events-none z-[9999] mix-blend-difference" />
      <div ref={trailerRef} className="fixed top-0 left-0 w-10 h-10 border border-cyan-500/50 rounded-full pointer-events-none z-[9998] transition-all duration-300 ease-out flex items-center justify-center custom-cursor-trailer" />
      <style>{`
...
// 3. UI Components with new interactions
...
const ProjectPreview = ({ activeImage, mousePos }) => {
  ...
  // Logic to ensure the preview stays within viewport
  const style = {
    top: `${mousePos.y}px`,
    left: `${mousePos.x + 40}px`, // Offset to right
  ...
  return (
    <div 
      ...
      style={style}
    >
      <div className="w-64 h-40 bg-black/80 rounded-lg overflow-hidden border-2 border-cyan-500 shadow-[0_0_30px_rgba(6,182,212,0.4)] relative">
        {/* Fallback to placeholder if image not found logic is tricky in pure client-side without config, assumes public folder */}
        <img 
          src={activeImage} 
          alt="Project Preview" 
        ...
        <div className="absolute bottom-2 left-2 text-xs text-cyan-300 font-mono">PREVIEW MODE</div>
      </div>
    </div>
  );
};
    ...
    <div className="min-h-screen text-white font-sans selection:bg-cyan-500/30 selection:text-cyan-100 cursor-none">
      ...
      <section id="home" className="relative h-screen flex items-center justify-center px-6 pt-16">
        <div className="max-w-4xl mx-auto text-center z-10">
          <div className="inline-block mb-4 px-4 py-1 rounded-full bg-white/5 border border-white/10 backdrop-blur-sm animate-fade-in-down">
      ...
      {/* Experience Section (Unchanged logic, simplified display) */}
...
Gained hands-on experience with LLMs, diffusion models, and practical creative AI workflows.</p>
      ...
      {/* Projects Section (COMPLETELY REVAMPED) */}
